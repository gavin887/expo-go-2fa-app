...
export function ClipboardAddScreen({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      let data = parseOtpauthUrl(normalized);
      if (!data && isValidBase32(normalized)) {
        data = {
          secret: normalized.replace(/\s/g, '').toUpperCase(),
          issuer: '',
      ...
      if (data) {
        setParsedData({
          ...data,
          secret: data.secret ? String(data.secret).replace(/\s/g, '').toUpperCase() : data.secret,
        });
      } else {
        setError('无法识别剪贴板内容');
      }
